...
class RGCNLayer(nn.Module):
    ...
    def forward(self, g, prev_h=[]):
        ...
        if len(prev_h) != 0 and self.skip_connect:
            ...
            skip_weight = torch.sigmoid(torch.mm(prev_h, self.skip_connect_weight) + self.skip_connect_bias)     # 使用sigmoid，让值在0~1
            # print("skip_ weight")
            # print(skip_weight)
...
class RGCNBlockLayer(RGCNLayer):
    ...
    def msg_func(self, edges):
        ...
        # print(pd.Series(edges.data['type'].tolist()).value_counts())
